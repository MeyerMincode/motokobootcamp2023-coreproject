...
actor Dao{
    ...
    let mbtCanister : actor {  icrc1_balance_of : ({owner:Principal; subaccount:?[Nat8]}) -> async Nat; } = actor("db3eq-6iaaa-aaaah-abz6a-cai");

    func checkBalanceOfVoter(caller : Principal, subaccount : ?[Nat8]) : async Nat {

        return await mbtCanister.icrc1_balance_of({owner = caller; subaccount = subaccount});
    };
    ...
    public shared ({caller}) func vote(id : Nat, answer : Bool) : async {#ok : Text; #error : Text}{
        if(Principal.isAnonymous(caller))
        {
            ...
            let callerDeposit = await checkBalanceOfVoter(caller, null);
